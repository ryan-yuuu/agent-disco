...
- **`calfkit-bridge`** — the single Discord gateway. Its calfkit `Client` calls
  ...
  `discord_list_channels` and `discord_read_messages` tools. Those tools are
  part of the default live function-tool surface (omitted `tools:` discovers
  them like any other builtin); only the bridge hosts them because it alone
  holds the bot token. The bridge owns
