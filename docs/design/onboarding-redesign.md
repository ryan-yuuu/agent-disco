...
- **Autorestart:** `_worker_runtime.run_worker_until_signal()` forces a *non-zero exit on a clean,
  signal-less exit* — but ONLY tools/mcp/router use it. The **bridge and agents exit 0** on a clean return,
  so `restart: on_failure` would not restart them. → bridge + agents use `restart: always` (suppressed on
  intentional `stop`). See **§12.0 / §12.1**.
...
- **Restart policy:** bridge + agents → `restart: always` (they exit 0 on a clean signal-less return and do
  NOT use `run_worker_until_signal`; only tools/mcp/router do). `stop` suppresses restart via termination.
...
- **restart:** bridge+agents → `availability.restart: always` (backoff 2, max_restarts 0) — they exit 0 on
  clean return; tools/mcp/router → `on_failure` (they use `run_worker_until_signal` → non-zero). NEVER
  `exit_on_failure`. Intentional stop (`down` / `PATCH /process/stop`) does not auto-restart.
