...
The original ``calfcord.tools`` package required every new
...
of :mod:`calfcord.tools.__init__`. That coupling is a paper
...
def _clone_with_name(node: ToolNodeDef, new_name: str) -> ToolNodeDef:
    ...
    * ``node_id`` — forensic logging identity
    ...
    try:
        ...
        # bare TypeError propagating from ``calfcord.tools``
