...
class TestRenderRow:
    ...
    def test_pending_consult_with_a_request_preview_shows_the_ask_inline(self) -> None:
        # A NESTED consult renders in the caller's trace INSIDE the same audit
        # thread, so its exchange sits right below — there is no separate thread
        # to link. The preview shows a glimpse of the ask in the link's slot; a
        # None thread_url here is deliberate, NOT an audit gap.
        row = ConsultRow(key="c1", peer="terra", request_preview="review the auth changes in src")
        assert render_row(row) == '◐ consulting terra · "review the auth changes in src"'

    def test_resolved_consult_keeps_the_request_preview(self) -> None:
        row = ConsultRow(key="c1", peer="terra", request_preview="review the auth changes", state="ok")
        assert render_row(row) == '-# ● consulted terra · "review the auth changes"'

    def test_denied_consult_with_a_preview_appends_it_after_the_reason(self) -> None:
        # The preview takes the link's slot (same `·` separator), so the denied
        # reason and the preview coexist without a doubled em-dash.
        row = ConsultRow(
            key="c1", peer="terra", state="denied", denial_reason="cycle detected", request_preview="review the auth"
        )
    ...
    def test_request_preview_is_hygienised(self) -> None:
        ...
        row = ConsultRow(key="c1", peer="terra", request_preview="line1\n*bold*")
...
class TestGrowthReserve:
    ...
    def test_no_resolved_state_grows_a_consult_row_with_a_preview_beyond_the_reserve(self) -> None:
        ...
        pending = ConsultRow(key="c1", peer="conan", request_preview="p" * 60)
        for state in ("ok", "failed", "denied", "interrupted"):
            resolved = dataclasses.replace(pending, state=state, denial_reason=self._NOTE)  # type: ignore[arg-type]
            growth = self._growth(pending, resolved)
