...
class TestToolsWiring:
    ...
    *selectors*, resolved per turn against the capability view — never against a
    local registry at build time. Omitted ``tools:`` discovers every live
    function-tool node (including bridge-hosted Discord reads); an explicit
    builtin list becomes one ``Tools(names=[...])``.
    ...
    def test_omitted_tools_yields_discover_selector(self) -> None:
        """Omitted tools use calfkit's full function-tool discovery selector."""
